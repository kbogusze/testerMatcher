...
public class TesterRank {
    ...
    public static List<TesterScore> makeTesterReport(List<Tester> testerList, List<Device> devices){
        List<TesterScore> result = new ArrayList<>();

        for(Tester t : testerList)
        {
            result.add(makeTesterResumeFor(t,devices));
        }
        result.sort((o1, o2) -> o2.getBugNoReported().compareTo(o1.getBugNoReported()));
        return result;
    }

    public static TesterScore makeTesterResumeFor(Tester t, List<Device> devices) {
